...
class MaskVisionTransformer(nn.Module):
    def __init__(self, input_resolution: int, patch_size: int, width: int, layers: int, heads: int, output_dim: int):
        ...
        self.mask_prompt_depth = 3 
        self.mask_embedding = nn.Parameter(torch.zeros(self.mask_prompt_depth, self.input_resolution // patch_size * self.input_resolution // patch_size, width))

        for parameters in [self.mask_embedding]:
            nn.init.normal_(parameters, std=0.02)
    ...
    def forward(self, x: torch.Tensor, mask: torch.Tensor = None, return_token=False, pos_embedding=False):
        x = self.conv1(x)  # shape = [*, width, grid, grid]
        x = x.reshape(x.shape[0], x.shape[1], -1)  # shape = [*, width, grid ** 2]
        x = x.permute(0, 2, 1)  # shape = [*, grid ** 2, width]
        
        if mask is not None:
            mask = self.mask_pool(mask.to(x.dtype).squeeze()).reshape(mask.shape[0], -1).unsqueeze(-1)
        ...
        if mask is not None:
            for i, blk in enumerate(self.transformer.resblocks):
                ...
                if d < self.mask_prompt_depth:
                    masked_x = x[1:, :, :] * mask.permute(1, 0, 2) + \
                               mask_embedding[d].unsqueeze(0).permute(1, 0, 2) * (1 - mask.permute(1, 0, 2))
...
class CLIP(nn.Module):
    ...
    def encode_image(self, image, pos_embedding, mask):
        return self.visual(image.type(self.dtype), pos_embedding=pos_embedding, mask=mask)
    ...
    def forward(self, image, text, pos_embedding=False, mask=None):
        image_features = self.encode_image(image, pos_embedding, mask)
